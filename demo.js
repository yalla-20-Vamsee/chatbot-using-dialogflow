...
async function authenticate(agent){
    ...
    if (snap==null){
        ...
        username=snap.phn;
        agent.add('you are regiersed, please type in your complaint');
    }
}
...
async function authenticatestatus(agent){
    ...
    if (snap==null){
        ...
        username=snap.phn;
        agent.add('you are regiersed, now type in your ticket number');
    }
}
async function fetchstatus(agent){
    console.log(agent.query);
    const client=new MongoClient(url);
    await client.connect();
...
app.post("/dialogflow", express.json(), (req, res) => {
    ...
    intentMap.set("fetchstatus",fetchstatus);
